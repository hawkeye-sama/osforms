...
Every request includes an `X-osforms-Signature` header with the raw hex digest (no `sha256=` prefix):

```
X-osforms-Signature: <hex>
```

The signature is computed as `HMAC-SHA256(requestBody, signingSecret)` where `requestBody` is the raw JSON string, and the digest is hex-encoded.
