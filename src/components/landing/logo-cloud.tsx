...
const LOGOS = [
  { 
    name: 'React', 
    icon: (
      <svg className="size-6 text-[#61DAFB]" viewBox="-11.5 -10.23174 23 20.46348" fill="none">
    ...
    icon: (
      <svg className="size-6 text-white" viewBox="0 0 180 180" fill="none">
        ...
        <g mask="url(#mask0)">
          ...
          <path d="M149.508 157.52L69.142 54H54V125.97H66.1136V69.1836L134.635 157.49C139.728 154.517 144.694 151.191 149.508 147.52V157.52Z" fill="white"/>
  ...
  { 
    name: 'Svelte', 
    icon: (
      <svg className="size-6 text-[#FF3E00]" viewBox="0 0 24 24" fill="currentColor">
        ...
        <path d="M11.5 13.55l-5.75-3.32c-.15-.09-.3-.11-.46-.07-.15.03-.29.13-.37.26l-3.32 5.75c-.09.15-.11.3-.07.46.03.15.13.29.26.37l5.75 3.32c.15.09.3.11.46.07.15-.03.29-.13.37-.26l3.32-5.75c.09-.15.11-.3.07-.46-.03-.15-.13-.29-.26-.37z" opacity=".5"/>
      </svg>
    )
  },
  { 
    ...
    icon: (
      <svg className="size-6" viewBox="0 0 256 221" fill="none">
        ...
        <path d="M157.44 0H204.8L128 132.48L51.2 0H98.56L128 50.88L157.44 0Z" fill="#35495E"/>
      </svg>
    )
  },
  { 
  ...
  { 
    name: 'Astro', 
    icon: (
      <svg className="size-6 text-white" viewBox="0 0 256 366" fill="currentColor">
  ...
  return (
    ...
    >
      <motion.p 
        ...
        className="text-muted-foreground mb-10 text-xs font-semibold uppercase tracking-[0.2em]"
      >
        Works with your favorite tools
      </motion.p>
      <div className="flex flex-wrap justify-center gap-10 md:gap-16 opacity-40 grayscale hover:opacity-100 hover:grayscale-0 transition-all duration-700">
        {LOGOS.map((logo, i) => (
          <motion.div
            key={logo.name}
            variants={{
